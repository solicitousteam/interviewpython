...
def next_question():
    ...
    index = len(partial_matches) // 20
    
    # Ensure the index is within the valid range
    if index >= len(partial_matches):
        index = len(partial_matches) - 1
    
    # Select the sub-category based on the calculated index
    sub_category = partial_matches[index]
